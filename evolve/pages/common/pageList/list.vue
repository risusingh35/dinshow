	...
	<div class="evolve-page">
		<div class="evolve-page-header">
			<div class="evolve-page-header-icons">
				<a class="sc-actions-icon mdi mdi-refresh  md-color-light-green-600" @click="onClickRefresh()"></a>
...
export default {
	...
	created: async function () {
		...
		this.getAllPageDetails(this.params.customParams);
	...
	methods: {
		...
		downloadPdf: function () {
			...
			this.pdfData = {
				...
				logo: img,
				printData: {
					title: `Evolve ${this.params.pageTitle}`,
					tableData: {
						header: header,
						data: data,
					},
					fileName: `Evolve ${this.params.pageTitle}`,
				},
			};
		},

		downloadCsv: function () {
			let filename = `Evolve ${this.params.pageTitle}`;
			let csv = [];
			let html = document.getElementById("pageDataTable").outerHTML;
			let rows = document.querySelectorAll("table tr");
			for (let i = 0; i < rows.length; i++) {
		...
		getAllPageDetails : async function (pageCode) {
			try {
				...
				this.notification("danger", 3000, 'Error While Get Page Details!!');
			}
		...
		getPageDataList: async function () {
			try {
				...
				this.notification("danger", 3000, 'Error While Get Page Data List!!');
			}
		...
		onClickRefresh : async function () {
			try {
				...
				this.notification("danger", 3000, "Error While Refresh the Page Data");
			}
		},
		...
		onClickViewButton: async function (data) {
			...
			this.$store.dispatch('addNewTab', {
				title: `View ${this.params.pageTitle}`, 
		...
		onClickDelete : async function (data) {
			try {
				...
				UIkit.modal.confirm("ARE YOU SURE TO DELETE THIS RECORD").then(async function () {
					...
					let list = await thisl.$axios.$post("/api/v1/Common/pageList/deletePageRow", data).catch((e) => { thisl.notification("danger", 3000, "Problem With Connecting Server!!");
					});
					if (list.statusCode == 200) {
						thisl.notification("success", 3000, list.message);
						thisl.getPageDataList()
					} else {
						thisl.notification("danger", 3000, list.message);
					}
				});
			} catch (error) {
				console.log("error", error.message);
		...
		onClickEdit : async function (data) {
			try {
				...
				this.$store.dispatch('addNewTab', {
					title: `Edit ${this.params.pageTitle}`, 
				...
				console.log("error", error.message);
				this.notification("danger", 3000, 'Error While Edit Data!!');
			}
		},
		...
		onClickAdd : async function () {
			try {
				...
				this.$store.dispatch('removeOneTab', '/common/pageList/options')
				this.$store.dispatch('addNewTab', {
					title: `Add ${this.params.pageTitle}`, 
					url: '/common/pageList/options',
					params: {
						rowData : {},
						columnList : this.allColumnList,
				...
				console.log("error", error.message);
				this.notification("danger", 3000, 'Error While Add Data!!');
			}
		}
