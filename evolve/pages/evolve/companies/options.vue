				...
				<div class="sc-top-bar-content uk-flex uk-flex-1">
					...
					<div class="sc-actions uk-margin-left">
						...
						<button
							...
							class="sc-button datatable-print-button sc-button-primary"
							@click="$destroy(); $store.dispatch('removeOneTab', '/evolve/companies/options')"
						>
							<i class="mdi mdi-keyboard-backspace"></i>
							{{ translate.back }}
						</button>
				...
				<div class="uk-child-width-1-3@m uk-grid" data-uk-grid>
					...
					<div>
						<button
							type="button"
							class="sc-button"
							@click="createCompany($event)"
						>
							{{ translate.create_company }}
						</button>

						<!-- <nuxt-link to="/evolve/companies/list">
							<button type="button" class="sc-button sc-button-danger uk-modal-close">
								{{ translate.cancel }}
							</button>
            </nuxt-link>-->
		...
		async createCompany (e) {
            ...
            '<div class="uk-flex uk-flex-middle"><span class="uk-flex-1 uk-text-center">Please Fill Require Field(s)</span></div>',
				...
				let response = await this.$axios
					.$post("/api/v1/evolve/company/createCompany", companyDetails)
					.catch((e) => {
						this.notification(
							"danger",
							3000,
