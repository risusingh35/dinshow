	...
	<div class="evolve-page">
		<div class="evolve-page-header">
			<div class="evolve-page-header-icons">
				<a class="sc-actions-icon mdi mdi-refresh  md-color-light-green-600" @click="getCompanyList()"></a>
			...
			<div class="evolve-page-header-icons evolve-float-right"> 
				<a href="javascript:void(0)" class="sc-actions-icon mdi mdi-close-box md-color-red-600" @click="$store.dispatch('removeOneTab', '/evolve/companies/listV2')"></a>
			</div>
									...
									<table id="menu_type" class="uk-table uk-table-striped">
										<thead>
											<tr>
												...
												<th>{{ translate.company_domain }}</th>
												<th>{{ translate.business_Group_code }}</th>
												<th>{{ translate.aaddress_code }}</th>
										...
										<tbody>
											<tr v-for="(row, index) in CompanyList" :key="index">
												<td>{{ row.EvolveCompany_Code }}</td>
												<td>{{ row.EvolveCompany_Domain }}</td>
												<td>{{ row.EvolveBusinessGroup_Domain }}</td>
												<td>{{ row.EvolveAddress_Code }}</td>

												<td>
													<!-- {{ row.EvolveCompany_IsActive }} -->
													<span v-if="row.EvolveCompany_IsActive == 1">
												...
												<!-- <td>
													<button
														...
														@click="onCreateOrEditCompany(row.EvolveCompany_ID)"
													>
	...
	data () {
		return {
			translate: {
				...
				options : "Options",
				business_Group_code : 'Buisness Group Code	'
	...
	methods: {
		...
		getCompanyList: async function () {
			let list = await this.$axios
				.$post("/api/v1/evolve/Company/getCompanyListV2", {
			...
			if (list.statusCode == 200) {
				...
				this.CompanyList = list.result.records;
		...
		onCreateOrEditCompany: async function (EvolveCompany_ID) {
			// this.$router.push(this.editUrl + EvolveSalesQuote_ID);

			this.$store.dispatch('addNewTab', {
				title: EvolveCompany_ID == '' ? this.translate.create_company  :  this.translate.update_company, 
				url: '/evolve/companies/optionsV2',
				params: {
		...
		onDeleteBusinessGroup: async function (index) {
			...
			UIkit.modal
				.confirm(
					"Are You Sure To Delete Menu Type " +thisl.CompanyList[index].EvolveBusinessGroup_Name +" ?")
				.then(
					async function () {
						...
						let response = await thisl.$axios
							.$post("/api/v1/evolve/MenuType/deleteMenuType", {
								EvolveCompany_ID : thisl.CompanyList[index].EvolveCompany_ID,
							})
							.catch((e) => {
								thisl.notification(
									"danger",
