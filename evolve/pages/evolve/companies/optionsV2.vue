	...
	<div class="evolve-page">
		<div class="evolve-page-header">
			...
			<div class="evolve-page-header-icons evolve-float-right"> 
				<a href="javascript:void(0)" class="sc-actions-icon mdi mdi-close-box md-color-red-600" @click="$destroy() ; $store.dispatch('removeOneTab', '/evolve/companies/optionsV2')"></a>
			</div>
		</div>
					...
					<div class="uk-modal-body">
						<div class="uk-grid" data-uk-grid>
							<div class="uk-width-1-2@m">
								<div class="uk-grid" data-uk-grid>
									<div class="uk-width-1-2@m">
										<label for="table lable" class="evolve-input-lable">{{ translate.business_Group }} :</label>
									</div>
									<div class="uk-width-1-2@m">
										<Select2
											v-model="$v.businessGroup.$model"
											:settings="{ 'width': '100%', 'placeholder': translate.business_Group, allowClear: true }"
											name="businessGroup"
											:error-class="$v.businessGroup.$error"
											:validator="$v.businessGroup"
											class="evolve-input"
											tabindex="1"
											autofocus	
											@change="onChangeBuisnessGroup()"
										>
											<option key="" value="">
											</option>
											<option
												v-for="bgl in BusinessGroupList"
												:key="bgl.EvolveBusinessGroup_ID"
												:value="bgl.EvolveBusinessGroup_ID"
											>
												{{ bgl.EvolveBusinessGroup_Name }} - {{ bgl.EvolveBusinessGroup_Code }}
											</option>
										</Select2>
									</div>
								</div>
							...
							<div class="uk-width-1-2@m">
								<div class="uk-grid" data-uk-grid>
									<div class="uk-width-1-2@m">
										<label for="table lable" class="evolve-input-lable">{{ translate.company_code }} :</label>
									</div>
									<div class="uk-width-1-2@m">
										<ScInput
											v-model="$v.companycode.$model"
											mode="outline"
											:placeholder="translate.company_code"
											name="companycode"
											:error-class="$v.companycode.$error"
							...
							<div class="uk-width-1-2@m">
								<div class="uk-grid" data-uk-grid>
									<div class="uk-width-1-2@m">
										<label for="table lable" class="evolve-input-lable">{{ translate.company_name }} :</label>
									</div>
									<div class="uk-width-1-2@m">
										<ScInput
													...
													<div class="uk-grid" data-uk-grid>
														...
														<div class="uk-width-1-2@m">
															...
															>
																...
																<option
																	v-for="taxzone in TaxZoneList"
																	:key="taxzone.EvolveGenericCodeMaster_Value"
																	:value="taxzone.EvolveGenericCodeMaster_Value"
																>
																	{{ taxzone.EvolveGenericCodeMaster_Value }}
																</option>
															</Select2>
														</div>
													</div>
										...
										<li>
											<div class="uk-grid" data-uk-grid>
												...
												<div class="uk-width-1-3@m">
													<div class="uk-grid" data-uk-grid>
														<div class="uk-width-1-2@m">
															<label for="table lable" class="evolve-input-lable">{{ translate.contact_number }} :</label>
														</div>
														<div class="uk-width-1-2@m">
															<ScInput
								...
								<div class="uk-width-1-2@m">
									<div class="uk-modal-footer uk-margin-top uk-text-right">
										<button
											...
											type="button"
											@click="$destroy() ; $store.dispatch('removeOneTab', '/evolve/companies/optionsV2')"
										>
											{{ translate.cancel }}
										</button>
										<button class="sc-button" type="button" @click="createOrUpateCompany()">
											{{ translate.save }}
										</button>
									</div>
								</div>
...
export default {
	...
	created: async function () {
		...
		this.getTaxZoneList();
		this.companyid = this.params.EvolveCompany_ID;
	...
	methods: {
		...
		getTaxClassList : async function (){
			...
			if (details.statusCode == 200) {
				console.log("details.result taxclass:::::::::::", details.result)
		...
		getBusinessGroupList : async function (){
			let details = await this.$axios
				.$post("/api/v1/evolve/Company/getBusinessGroupList")
				.catch((e) => {
					this.notification(
						"danger",
						3000,
			...
			if (details.statusCode == 200) {
				console.log("details.result:::::::::::", details.result)
				this.BusinessGroupList = details.result
			} else {
				this.notification("danger", 3000, details.message);
			}
		},
		...
		createOrUpateCompany: async function () {
			this.$v.$touch();
			if (this.$v.$invalid) {
				...
				let data = {
					EvolveCompany_ID : this.companyid,
					EvolveCompany_Code: this.companycode,
				...
				if(this.companyid == ''){
					...
					createOrEdit = await this.$axios.$post("/api/v1/evolve/Company/createCompanyV2", data)
						.catch((e) => {
							this.notification(
								"danger",
								3000,
					...
					createOrEdit = await this.$axios.$post("/api/v1/evolve/Company/upateCompanyV2", data)
						.catch((e) => {
							this.notification(
								"danger",
								3000,
				...
				if (createOrEdit) {
					if (createOrEdit.statusCode == 200) {
						this.$destroy() ;
						this.$store.dispatch('removeOneTab', '/evolve/companies/optionsV2')
		...
		onChangeBuisnessGroup: async function () {

			console.log("ON CHANGE BUISNESS GR" )

			if(this.businessGroup != '' && this.businessGroup != null ){


				let details = await this.$axios
					.$post("/api/v1/evolve/businessGroup/getSinglebusinessGroupDetails", {
						EvolveBusinessGroup_ID: this.businessGroup,
					})
					.catch((e) => {
						this.notification(
				...
				if (details.statusCode == 200) {
					...
					if(details.result.length > 0){
						...
						// this.BusinessGroupID = details.result[0].EvolveBusinessGroup_ID;
						// this.groupcode = details.result[0].EvolveBusinessGroup_Code;
						// this.groupname = details.result[0].EvolveBusinessGroup_Name;
						// this.addressId =  details.result[0].EvolveAddress_ID;
						this.Address1 = details.result[0].EvolveAddress_Address1;
						this.Address2 = details.result[0].EvolveAddress_Address2;
						this.Address3 = details.result[0].EvolveAddress_Address3;
