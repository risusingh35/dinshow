					...
					<div class="evolve_table_list">
						...
						<client-only>
							<table id="evolveMenuMaster" class="uk-table uk-table-striped">
								...
								<tbody>
									<tr v-for="(ml,index) in menuList" :key="index">
										<td>{{ ml.EvolveApp_Name }}</td>
										<td>{{ ml.EvolveMenu_Name }}</td>
										<td>{{ ml.EvolveMenu_Desc }}</td>

										<td>
											<span v-if="ml.EvolveMenu_IsActive == 1">
										...
										<td>
											<span class="uk-nav-icon">
												<i :class="ml.EvolveMenuType_Icon"></i>
											</span>
										</td>
										<td>{{ ml.EvolveMenu_Url }}</td>
										<td>
											<span v-if="ml.EvolveMenu_Parent == 0">Parent</span>
											<span v-else>{{ ml.parent }}</span>
										</td>
										<td>
											<button
												title="Edit"
												class="sc-button sc-button-primary waves-effect sc-button-mini waves-button waves-light"
												@click="onCreateOrEditMenu(ml.EvolveMenu_Id)"
											>
...
export default {
	...
	methods: {
		...
		onCreateOrEditMenu: async function (EMenuID) {
			...
			this.$store.dispatch('addNewTab', {
				title: EMenuID == '' ? 'Menu Create'  :  'Menu Update', 
