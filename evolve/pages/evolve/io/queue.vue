									...
									<div>
										<client-only>
											...
											<ScInput
												id="dateRange"
												v-model="dateRange"
												v-flatpickr="{ 'altInput': false, 'dateFormat': 'd-m-Y','allowInput': false }"
												name="dateRange"
													...
													<tr v-for="(iod,index) in IoData" :key="index">
														...
														<td>
															<button
																title="Edit"
																class="sc-button sc-button-primary waves-effect sc-button-mini waves-button waves-light"
																@click="getIoFileData(index)"
															>
																{{ translate.file_data }}
															</button>
															<!-- <a
																target="_blank"
																:href="iod.IOSERVERURL+'../'+ iod.EvolveIO_FileName"
																class="sc-button sc-button-primary waves-effect sc-button-mini waves-button waves-light"
															>
	...
	methods: {
		...
		async getIoData () {
			const getIoData = await this.$axios
				.$post("/api/v1/evolve/IoQueue/getIoReportData", {
		...
		getErrMsg : async function (data){
			this.errmessage = data
			UIkit.modal('#err_msg').show(data);
		},
		getIoCodeData: async function (id) {
			this.ioDataId = id;
			let ioCodeData = await this.$axios.$post(
				"/api/v1/evolve/IoQueue/getSingleIoCodeData",
				{ id: this.ioDataId }
			);
			this.codeData = ioCodeData.result.EvolveIO_Data;
			if (
				/^[\],:{}\s]*$/.test(
					this.codeData
						.replace(/\\["\\\/bfnrtu]/g, "@")
				...
				this.codeData = JSON.parse(ioCodeData.result.EvolveIO_Data);
				this.isJson = true;
			} else {
				this.codeData = ioCodeData.result.EvolveIO_Data;
				this.isJson = false;
		...
		getIoFileData: async function (index) {
			let path = this.IoData[index].IOSERVERURL + '../' + (this.IoData[index].EvolveIO_FileName).replace("./ERP/", "" );
			// let path = this.IoData[index].IOSERVERURL + '../' + './ERP/IN/112021/EvolveState.qxtendRecieve.1637047463817.json' 
			window.open(path);
		},
		...
		editQueue: async function (id) {
			let thisl = this;
			UIkit.modal
				.confirm("Are You Sure to Re-Queue .......!")
				.then(async function () {
					let changeStatus = await thisl.$axios.$post(
						"/api/v1/evolve/IoQueue/changeIoCodeStatus",
						{ id: id }
					);

					if (changeStatus.statusCode == 200) {
						thisl.notification("success", 3000, changeStatus.message);
						thisl.getIoData();
					} else {
						thisl.notification("success", 3000, changeStatus.message);
					}
		...
		async downloadPdf () {
			...
			let html = document.getElementById("evolveIoQueue").outerHTML;
			...
			this.pdfData = {
				...
				logo: img,
				printData: {
					title: "Evolve IO Queue",
					tableData: {
						header: header,
						data: data,
					},
					fileName: "EvolveIoQueue",
				},
			};
		},
		async downloadCsv () {
			let filename = "EvolveIoQueue";
			let csv = [];
			let html = document.getElementById("evolveIoQueue").outerHTML;
			let rows = document.querySelectorAll("table tr");
			for (let i = 0; i < rows.length; i++) {
