	...
	<div class="evolve-page">
		<div class="evolve-page-header">
			<div class="evolve-page-header-icons">
				<a class="sc-actions-icon mdi mdi-refresh  md-color-light-green-600" @click="getAllMenuTypeList()"></a>
			...
			<div class="evolve-page-header-icons evolve-float-right"> 
				<a href="javascript:void(0)" class="sc-actions-icon mdi mdi-close-box md-color-red-600" @click="$destroy(); $store.dispatch('removeOneTab', '/evolve/menuType/list')"></a>
			</div>
			...
			<div class="evolve-page-header-icons evolve-float-right"> 
				<button
					...
					@click="onCreateOrEditMenuType('')"
				>
					{{ translate.create_menu_type }}
				</button>
			</div>
							...
							<div class="uk-overflow-auto">
								...
								<client-only>
									<table id="menu_type" class="uk-table uk-table-striped">
										...
										<tbody>
											<tr v-for="(row, index) in menuTypeList" :key="index">
												<td>{{ row.EvolveMenuType_Type }}</td>
												<td>{{ row.EvolveMenuType_Description }}</td>
												<td>	<i :class="row.EvolveMenuType_Icon"></i></td>
												<td>
													<button
														title="Edit"
														class="sc-button sc-button-primary sc-button-mini waves-effect sc-button-mini waves-button waves-light"
														data-uk-toggle="target: #addEditAppMaster"
														@click="onCreateOrEditMenuType(row.EvolveMenuType_ID)"
													>
...
export default {
	...
	data () {
		...
		return {
			...
			translate: {
				...
				icon :"Icon",
				create_menu_type :"CREATE MENU TYPE",
				description :"Description",
				search_icon :"Search Icon",
				search_icon :"Selected Icon",
				search_here : 'Search Here',
				menu_type_update : 'Menu Type  Update',
	...
	methods: {
		...
		getAllMenuTypeList: async function () {
			let list = await this.$axios
				.$post("/api/v1/evolve/MenuType/getAllMenuTypeList", {
			...
			if (list.statusCode == 200) {
				this.menuTypeList = list.result.records;
		...
		onCreateOrEditMenuType: async function (EvolveMenuType_ID) {
			// this.$router.push(this.editUrl + EvolveSalesQuote_ID);
			this.$root.$emit("onCloseTabCalled", '/evolve/menuType/options');

			this.$store.dispatch('addNewTab', {
				title: EvolveMenuType_ID == '' ? this.translate.menu_type_create  :  this.translate.menu_type_update, 
				url: '/evolve/menuType/options',
				params: {
					EvolveMenuType_ID: EvolveMenuType_ID,
				}
			});
		},
		onDeleteMenuType: async function (index) {
			let thisl = this;
			UIkit.modal
				.confirm(
