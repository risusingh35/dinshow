	...
	<div class="evolve-page">
		<div class="evolve-page-header">
			<div class="evolve-page-header-icons">
				<a class="sc-actions-icon mdi mdi-refresh  md-color-light-green-600" @click="getProjectList()"></a>
			...
			<div class="evolve-page-header-icons evolve-float-right"> 
				<a href="javascript:void(0)" class="sc-actions-icon mdi mdi-close-box md-color-red-600" @click="$store.dispatch('removeOneTab', '/mdm/project/list')"></a>
			</div>
			<!-- 
			<div class="evolve-page-header-icons evolve-float-right"> 
				<button class="sc-button datatable-print-button" type="button" @click="onCreateOrEditProject('')">
					...
					<div class="uk-card">
						<div class="uk-card-body">
							<div class="uk-overflow-auto">
								...
								<client-only>
									<table id="ProjectTable" class="uk-table uk-table-striped">
										<thead>
											<tr>
										...
										<tbody>
											<tr v-for="(row, index) in ProjectList" :key="index">
												<td>{{ row.EvolveProject_Code }}</td>
...
export default {
	...
	data () {
		return {
			...
			pdfData: {},
			csvExportColums: [0, 1, 2, 3, 4],
			pdfExportColums: [0, 1, 2, 3, 4],
			search: "",
			/** End : EvolveDataTable */
		};
	},
	...
	methods: {
		...
		downloadPdf: function () {
			...
			let html = document.getElementById("ProjectTable").outerHTML;
			...
			this.pdfData = {
				...
				logo: img,
				printData: {
					title: "Evolve Project List",
					tableData: {
						header: header,
						data: data,
					},
					fileName: "EvolveProjectList",
				},
			};
		},

		downloadCsv: function () {
			let filename = "EvolveProjectList";
			let csv = [];
			let html = document.getElementById("ProjectTable").outerHTML;
			let rows = document.querySelectorAll("table tr");
			for (let i = 0; i < rows.length; i++) {
		...
		getProjectList: async function () {
			let list = await this.$axios.$post("/api/v1/eMdm/Project/getProjectList", {
			...
			if (list.statusCode == 200) {
				this.ProjectList = list.result.records;
		...
		onCreateOrEditProject: async function (EvolveAddress_ID) {
