...
module.exports = {
    ...
    //     try {
    //         let woList = await Evolve.App.Services.SmartFactory.PickList.SrvSoPickListGenerate.getSoList();
    ...
    //     try {
    //         let result = await Evolve.App.Services.SmartFactory.PickList.SrvSoPickListGenerate.WOAjaxUrl(req.body.term);
    ...
    getSoList: async function (req, res) {
        try {
            ...
            let woDetails = await Evolve.App.Services.SmartFactory.PickList.SrvSoPickListGenerate.getSoList(req.body.term);
    ...
    getLocationList: async function (req, res) {
        try {
            let locationList = await Evolve.App.Services.SmartFactory.PickList.SrvSoPickListGenerate.getLocationList();
    ...
    getSoDetails: async function (req, res) {
        try {
            ...
            let data = {};
            let details = await Evolve.App.Services.SmartFactory.PickList.SrvSoPickListGenerate.getSoDetails(req.body.EvolveSalesOrder_ID);
            if (details instanceof Error) {
                ...
                // } else {
                    ...
                    let bomDetails = await Evolve.App.Services.SmartFactory.PickList.SrvSoPickListGenerate.getSoLineDetails(req.body.EvolveSalesOrder_ID);
        ...
        try {
            ...
            if (req.body.EvolvePickListDetail.length == 0) {
                ...
                if (pickListNumber == undefined || pickListNumber.length == 0) {
                    ...
                    let details = await Evolve.App.Services.SmartFactory.PickList.SrvSoPickListGenerate.generatePickList(req.body);
                    ...
                    if (details instanceof Error || details.rowsAffected < 1) {
                        ...
                        for (let i = 0; i < req.body.EvolvePickListDetail.length; i++) {
                            ...
                            let pickListDetails = await Evolve.App.Services.SmartFactory.PickList.SrvSoPickListGenerate.addPickListDetails(req.body.EvolvePickListDetail[i]);
                        ...
                        if (errorMessage == '') {
                            let updateWorkOrder = await Evolve.App.Services.SmartFactory.PickList.SrvSoPickListGenerate.updateWorkOrderPickListStatus(req.body);
    ...
    //     try {
    //          let checkExistPick = await Evolve.App.Services.SmartFactory.PickList.SrvSoPickListGenerate.checkExistPick(req.body.EvolveProdOrders_ID);
    ...
    //             {
    //                 let issueDetail = await Evolve.App.Services.SmartFactory.PickList.SrvSoPickListGenerate.issueDetail(req.body.EvolveProdOrders_ID);
    ...
    //                 let pickedDetails = await Evolve.App.Services.SmartFactory.PickList.SrvSoPickListGenerate.issueDetailWhilePickGenerated(req.body.EvolveProdOrders_ID);
    ...
    getPalletDetails: async function (req, res) {
        try {
            ...
            let palletDetails = {}
            let availablePallets = await Evolve.App.Services.SmartFactory.PickList.SrvSoPickListGenerate.getAvailablePallets(req.body);
            if (availablePallets instanceof Error) {
                ...
                let pickedPallets = await Evolve.App.Services.SmartFactory.PickList.SrvSoPickListGenerate.getpickedPallets(req.body);
    ...
    pickPallets: async function (req, res) {
        try {
            ...
            let woNumber = await Evolve.App.Services.SmartFactory.PickList.SrvSoPickListGenerate.getWoNumber(req.body.EvolveProdOrders_ID);
            if (woNumber instanceof Error) {
                ...
                let locStatus = await Evolve.App.Services.SmartFactory.PickList.SrvSoPickListGenerate.getLocationStatus(req.body.pickToLoc);
                if (locStatus instanceof Error) {
                    ...
                    let checkPickNumber = await Evolve.App.Services.SmartFactory.PickList.SrvSoPickListGenerate.checkExistPick(req.body.EvolveProdOrders_ID);
                    if (checkPickNumber instanceof Error) {
                        ...
                        if (checkPickNumber.rowsAffected == 0) {
                            ...
                            if (pickListNumber == 0) {
                                ...
                                req.body.EvolvePickList_Number = pickListNumber
                                let createPickList = await Evolve.App.Services.SmartFactory.PickList.SrvSoPickListGenerate.createPickList(req.body) // get po barcode details 
                    ...
                    for (let i = 0; i < req.body.pickedPallets.length; i++) {
                        if (error == false) {
                            let changePalletStatus = await Evolve.App.Services.SmartFactory.PickList.SrvSoPickListGenerate.changePalletStatus(req.body.pickedPallets[i].EvolveInventory_ID, req.body);
                            if (changePalletStatus instanceof Error || changePalletStatus.rowsAffected < 1) {
                                ...
                                let addPickListDetails = await Evolve.App.Services.SmartFactory.PickList.SrvSoPickListGenerate.addPickListDetails(req.body.pickedPallets[i], req.body);
                                if (addPickListDetails instanceof Error || addPickListDetails.rowsAffected < 1) {
                                    ...
                                    // {
                                    //     let updatePickList = await Evolve.App.Services.SmartFactory.PickList.SrvSoPickListGenerate.updatePickList(req.body.EvolvePickList_ID,req.body.EvolveUser_ID ,req.body.pickedPallets[i].EvolveInventory_QtyOnHand);
                                    ...
                                    // }else
                                    // {
                                    let updatePickList = await Evolve.App.Services.SmartFactory.PickList.SrvSoPickListGenerate.updateBomPickList(req.body.selectedIssueLine, req.body.EvolveUser_ID, req.body.pickedPallets[i].EvolveInventory_QtyOnHand);
                                    if (updatePickList instanceof Error || updatePickList.rowsAffected < 1) {
                                        error = true;
    ...
    unpickPallets: async function (req, res) {
        try {
            req.body.EvolveUser_ID = req.EvolveUser_ID;
            let locStatus = await Evolve.App.Services.SmartFactory.PickList.SrvSoPickListGenerate.getLocationStatus(req.body.EvolveLocation_ID);
            if (locStatus instanceof Error) {
                Evolve.Log.error("EERR2800 : Error while get location status")
                let obj = { statusCode: 400, status: "fail", message: "EERR2800 : Error while get location status", result: null };
                res.send(obj);
                ...
                for (let i = 0; i < req.body.unPickedPallets.length; i++) {
                    if (error == false) {
                        let deletePicklist = await Evolve.App.Services.SmartFactory.PickList.SrvSoPickListGenerate.deletePicklist(req.body.unPickedPallets[i]);
                        if (deletePicklist instanceof Error || deletePicklist.rowsAffected < 1) {
                            ...
                            // if(req.body.isBom == false)
                            // {
                            //     let updatePickQty = await Evolve.App.Services.SmartFactory.PickList.SrvSoPickListGenerate.updatePickQty(req.body.unPickedPallets[i].EvolveInventory_QtyOnHand ,req.body.selectedIssueLine);
                            ...
                            //             res.send(obj);
                            //     }else{
                            //     let updatePallet = await Evolve.App.Services.SmartFactory.PickList.SrvSoPickListGenerate.updatePallet(req.body.unPickedPallets[i] , req.body.EvolveLocation_ID ,req.body.EvolveUser_ID);
                            ...
                            //     }
                            // }else{
                            let updatePickQty = await Evolve.App.Services.SmartFactory.PickList.SrvSoPickListGenerate.updateBomPickListOnUnpick(req.body.unPickedPallets[i].EvolveInventory_QtyOnHand, req.body.selectedIssueLine, req.body.EvolveUser_ID);
                            if (updatePickQty instanceof Error || updatePickQty.rowsAffected < 1) {
                                error = true
                                ...
                                let updatePallet = await Evolve.App.Services.SmartFactory.PickList.SrvSoPickListGenerate.updatePallet(req.body.unPickedPallets[i], req.body.EvolveLocation_ID, req.body.EvolveUser_ID, req.body.EvolveInventory_Status);
                                if (updatePallet instanceof Error || updatePallet.rowsAffected < 1) {
                                    error = true
    ...
    getSubItemList: async function (req, res) {
        try {
            let subItems = await Evolve.App.Services.SmartFactory.PickList.SrvSoPickListGenerate.getSubItemList(req.body.EvolveItem_ID);
    ...
    getSubItemAvailPallets: async function (req, res) {
        try {
            let pallets = await Evolve.App.Services.SmartFactory.PickList.SrvSoPickListGenerate.getAvailablePallets(req.body);
    ...
    getWobomIssueDetails: async function (req, res) {
        try {
            // let details = await Evolve.App.Services.SmartFactory.PickList.SrvSoPickListGenerate.getWobomIssueDetails(req.body.EvolveWoSchedule_ID);
            // if (details instanceof Error) {
            ...
            req.body.EvolveUser_ID = req.EvolveUser_ID;
            let error = false;
            let materialToIssue = await Evolve.App.Services.SmartFactory.PickList.SrvSoPickListGenerate.getWobomIssueDetails(req.body.EvolveWoSchedule_ID);
            if (materialToIssue instanceof Error) {
                ...
                for (let i = 0; i < materialToIssue.recordset.length; i++) {
                    if (error == false) {
                        ...
                        let subItems = await Evolve.App.Services.SmartFactory.PickList.SrvSoPickListGenerate.getSubItems(materialToIssue.recordset[i].EvolveSchedulingBom_CompItem_ID);
                        if (subItems instanceof Error) {
                            error = true;
                        } else {
                            for (let j = 0; j < subItems.recordset.length; j++) {
                                if (error == false) {
                                    let getQtyOnHAnd = await Evolve.App.Services.SmartFactory.PickList.SrvSoPickListGenerate.getQtyOnHAnd(subItems.recordset[j].EvolveSubItem_SubItem_ID, req.body.EvolveWoSchedule_ID)
                                    if (subItems instanceof Error) {
                                        error = true;
                                    } else {
    ...
    //     try {
    //         let scannedItem = await Evolve.App.Services.SmartFactory.PickList.SrvSoPickListGenerate.getScannedItem();
