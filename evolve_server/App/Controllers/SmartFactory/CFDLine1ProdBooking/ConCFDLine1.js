...
module.exports = {
    ...
    getWoListAndSectionCodeByMachinCode: async function(req, res) {
        try {
            let List = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.getWoListAndSectionCodeByMachinCode(req.body.EvolveMachine_Code);
    ...
    getSectionList: async function (req, res) {
        try {
            let list = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.getSectionList();
            if (list instanceof Error) {
    ...
    getMachineList: async function (req, res) {
        try {
            let list = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.getMachineList(req.body);
            if (list instanceof Error) {
    ...
    getWoList: async function (req, res) {
        try {
            let list = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.getWoList(req.body);
            if (list instanceof Error) {
    ...
    getWoDetails: async function (req, res) {
        try {
            let data = {};
            let details = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.getWoDetails(req.body.EvolveProdOrders_ID);
            if (details instanceof Error) {
                ...
                let getUOMDetails = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.getUOMDetails('KG');
                    let convDetails = {
                        EvolveUom_ID: getUOMDetails.recordset[0].EvolveUom_ID,
                        EvolveUom_AltID: details.recordset[0].EvolveUom_ID,
                        EvolveItem_ID: details.recordset[0].EvolveItem_ID,
                    }
                let convValue = await Evolve.App.Controllers.Common.ConCommon.getUomConversionValue(convDetails);
                console.log('convValue==',convValue)
                data.materialToProduce[0].convValue = convValue
                let bomDetails = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.getWoBomDetails(req.body.EvolveProdOrders_ID);
                if (bomDetails instanceof Error) {
                    ...
                    let materialIssued = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.getTransHistory(data.materialToProduce[0]);
                    if (materialIssued instanceof Error) {
                        ...
                        let materialProduced = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.getTransHistory(data.materialToProduce[0]);
    ...
    onInventoryIssue: async function (req, res) {
        try {
            let errorMessage = '';
            let list = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.checkInventory(req.body);
            if (list instanceof Error) {
                ...
                if (list.recordset[0].EvolveLocation_ID != req.body.EvolveLocation_ID) {
                    ...
                    if (isItemMatched) {
                        ...
                        let changeInvStatus = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.changeInventoryStatus(list.recordset[0]);
                        if (changeInvStatus instanceof Error || changeInvStatus.rowsAffected < 1) {
                            ...
                            let updateProdOrderDetails = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.updateProdOrderDetails(list.recordset[0]);
    ...
    getMachinePlanDetails: async function (req, res) {
        try {
            ...
            let details = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.getMachinePlanDetails(req.body);
            if (details instanceof Error) {
                ...
                //    let upcomingPlans = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.getUpcomingPlansDetails(req.body);
    ...
    getPlanMtIssueDetails: async function (req, res) {
        try {
            ...
            let planDetails = {};
            let error = false;
            let materialToIssue = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.getMaterialToIssue(req.body);
            if (materialToIssue instanceof Error) {
                ...
                for (let i = 0; i < materialToIssue.recordset.length; i++) {
                    if (error == false) {
                        ...
                        let subItems = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.getSubItems(materialToIssue.recordset[i].EvolveSchedulingBom_CompItem_ID);
                        if (subItems instanceof Error) {
                            error = true;
                        } else {
                            for (let j = 0; j < subItems.recordset.length; j++) {
                                if (error == false) {
                                    let getQtyOnHAnd = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.getQtyOnHAnd(subItems.recordset[j].EvolveSubItem_SubItem_ID, req.body.EvolveWoSchedule_ID)
                                    if (subItems instanceof Error) {
                                        error = true;
                                    } else {
                ...
                let inComPlans = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.getInComingPLans(req.body);
    ...
    addProdComments: async function (req, res) {
        try {
            req.body.EvolveUser_ID = req.EvolveUser_ID;
            let addComments = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.addProdComments(req.body);
    ...
    issuePallet: async function (req, res) {
        try {
            ...
            let woNumber = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.getWoNumber(req.body.EvolveWoSchedule_ID);
            if (woNumber instanceof Error) {
                ...
                let changePalletStatus = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.changeInvPalletStatus(req.body);
                if (changePalletStatus instanceof Error || changePalletStatus.rowsAffected < 1) {
                    error = true;
                } else {
                    let addIssuedPallet = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.addIssuedPallet(req.body);
                    if (addIssuedPallet instanceof Error || addIssuedPallet.rowsAffected < 1) {
                        error = true;
                    } else {
                        let updateProdOrd = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.updateProdOrderBom(req.body);
    ...
    getRtsLocationList: async function (req, res) {
        try {
            let list = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.getRtsLocationList(req.body);
            if (list instanceof Error) {
    ...
    rtsQty: async function (req, res) {
        try {
            ...
            let locStatus = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.getLocationStatus(req.body.EvolveLocation_ID);
            if (locStatus instanceof Error) {
                ...
                let updateIssuedQty = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.updateIssuedQty(req.body);
                if (updateIssuedQty instanceof Error || updateIssuedQty.rowsAffected < 1) {
                    ...
                    let updateProdOrder = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.updateProdOrderIssueQty(req.body);
                    if (updateProdOrder instanceof Error || updateProdOrder.rowsAffected < 1) {
                        ...
                        let updateInventoryPallet = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.updateInventoryPallet(req.body);
                        if (updateInventoryPallet instanceof Error || updateInventoryPallet.rowsAffected < 1) {
                            ...
                            let changePalletRtsStatus = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.changePalletRtsStatus(req.body);
                            ...
                            //    }
                            //    else {
                            //         let newPallet = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.createNewPallet(newPalletId , req.body);
    ...
    getProdBookingDetails: async function (req, res) {
        try {
            ...
            let uomList = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.getWoItemSecUomList(req.body);
            if (uomList instanceof Error) {
                ...
                let bookedList = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.getWoBookedList(req.body.EvolveWoSchedule_ID);
    ...
    completeProductionBooking: async function (req, res) {
        try {
            ...
            if (req.body.EvolveLocation_Code == null) {
                ...
                let findMachineLocation = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.findMachineLocation(req.body.EvolveMachine_Code);
                if (findMachineLocation instanceof Error) {
                    ...
                    let itemDefLacation = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.getItemDefaultLocation(req.body.EvolveItem_ID);
                        ...
                        try {
                            if (err) {
                                ...
                                if (result == 'error') {
                                    ...
                                    req.body.EvolveUser_ID = req.EvolveUser_ID == undefined ? null : req.EvolveUser_ID;
                                    req.body.EvolveUnit_ID = req.EvolveUnit_ID == undefined ? null : req.EvolveUnit_ID;
                                    let addInventory = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.addInventory(req.body);
                                    if (addInventory instanceof Error || addInventory.rowsAffected < 1) {

                                        errorMessage = 'Error While Add Inventory';

                                    } else {
                                        let updateProdOrderDetails = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.updateProdOrderCompletedQty(req.body);
                                        if (updateProdOrderDetails instanceof Error || updateProdOrderDetails.rowsAffected < 1) {
                                            ...
                                            if(Evolve.Config.CFDLINE1LABEL == '' || Evolve.Config.CFDLINE1LABEL == null || Evolve.Config.CFDLINE1LABEL == undefined || Evolve.Config.CFDLINE1LABEL == 'null' || Evolve.Config.CFDLINE1LABEL == 'undefined' || Evolve.Config.CFDLINE1LABEL == 'NOLABEL'){
                                            }else{
                                                let GetPrinterByMachine = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.GetPrinterByMachine(req.body.EvolveMachine_ID);
                                                if( GetPrinterByMachine.rowsAffected > 0){
                                                    let EvolvePrinter_Code = GetPrinterByMachine.recordset[0].EvolvePrinter_Code;
                                                    let sendPrintCommand = await Evolve.App.Controllers.Common.ConCommon.AddNewPrintQuee(EvolvePrinter_Code , Evolve.Config.CFDLINE1LABEL , req.body)
                                            ...
                                            //     let checkSameBatchQCExits = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.checkSameBatchQCExits(req.body);
                                            ...
                                            //         let addNewQcOrder = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.addNewQcOrder(req.body);
    ...
    deleteBookedPallet: async function (req, res) {
        try {
            let error = false;
            let updateQty = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.updateWoOrderQty(req.body, 'DELETEPALLET');
            if (updateQty instanceof Error || updateQty.rowsAffected < 1) {
                error = true;
            } else {
                let deletePallet = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.deleteProdOrderPallet(req.body.EvolveProdOrdersDetail_ID);
                if (deletePallet instanceof Error || deletePallet.rowsAffected < 1) {
                    error = true;
                } else {
                    let deleteInventory = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.deleteInventory(req.body.EvolveInventory_ID);
    ...
    updateBookedPallet: async function (req, res) {
        try {
            ...
            req.body.EvolveUnit_ID = req.EvolveUnit_ID;
            let error = false;
            let updateQty = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.updateWoOrderQty(req.body, 'UPDATEPALLET');
            if (updateQty instanceof Error || updateQty.rowsAffected < 1) {
                error = true;
            }
            else {
                let uodateDetails = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.updatePbPalletDetails(req.body);
                if (uodateDetails instanceof Error || uodateDetails.rowsAffected < 1) {
                    error = true;
                } else {
                    let updateInvPbPallet = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.updateInvPbPallet(req.body);
    ...
    getSlittingItemList: async function (req, res) {
        try {
            ...
            let productQuality = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.getProductQuality();
    ...
    //     try {
    //        let shiftList = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.getTsShiftList(req.body);
    //        let shifts = [];
    //        let count = 0;
    ...
    //                     let getShifTName = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.getShifTName(shiftList.recordset[i].EvolveMachineCalendar_Shift3StartTime);
    ...
    //               if(count != 5){
    //                 let getShifTName = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.getShifTName(shiftList.recordset[i].EvolveMachineCalendar_Shift2StartTime);
    //                 if (getShifTName instanceof Error ||  getShifTName.rowsAffected<1 ) {
    ...
    //               }
    //               if(count != 5){
    //                 let getShifTName = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.getShifTName(shiftList.recordset[i].EvolveMachineCalendar_Shift1StartTime);
    //                 if (getShifTName instanceof Error ||  getShifTName.rowsAffected<1 ) {
    ...
    //     try {
    //          let qty = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.getCurrentShiftQty(req.body);
    //          console.log("qty>>>>" , qty);
    ...
    getRtsReasonCodeList: async function (req, res) {
        try {
            let list = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.getRtsReasonCodeList();
            if (list instanceof Error) {
    ...
    getSubReasonCodeList: async function (req, res) {
        try {
            let list = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.getSubReasonCodeList(req.body);
            if (list instanceof Error) {
    ...
    getOperatorData: async function (req, res) {
        try {
            ...
            let userData = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.getOperatorData(req.body);
    ...
    addTimeSheet: async function (req, res) {
        try {
            ...
            let addTs = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.addTimeSheet(req.body);
    ...
    getTimesheetList: async function (req, res) {
        try {
            let addTs = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.getTimesheetList(req.body);
    ...
    getItemSecUomList: async function (req, res) {
        try {
            let addTs = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.getWoItemSecUomList(req.body);
            if (addTs instanceof Error) {
    ...
    deleteTimeSheet: async function (req, res) {
        try {
            let error = false;
            req.body.EvolveUser_ID = req.EvolveUser_ID;
            let deleteSheet = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.deleteTimeSheet(req.body.deleteSheetId);
            if (deleteSheet instanceof Error || deleteSheet.rowsAffected < 1) {
                ...
                for (let i = 0; i < req.body.updateArray.length; i++) {
                    if (error == false) {
                        let updateSheetOnDelete = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.updateSheetOnDelete(req.body.updateArray[i], req.body.EvolveUser_ID);
    ...
    addSubTimeSheet: async function (req, res) {
        try {
            ...
            let addTs = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.addTimeSheet(req.body.tsData);
            ...
            if (addTs instanceof Error) {
                ...
                for (let i = 0; i < req.body.updateArray.length; i++) {
                    if (error == false) {
                        let updateSheetOnAdd = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.updateSheetOnDelete(req.body.updateArray[i], req.EvolveUser_ID);
    ...
    completeJob: async function (req, res) {
        try {
            ...
            let compJob = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.completeJob(req.body);
    ...
    addEditedTimeSheet: async function (req, res) {
        try {
            ...
            req.body.tsData.EvolveTimesheet_StartDateTime = req.body.tsData.EvolveTimesheet_StartDateTime.slice(6, 10) + '-' + req.body.tsData.EvolveTimesheet_StartDateTime.slice(3, 5) + '-' + req.body.tsData.EvolveTimesheet_StartDateTime.slice(0, 2) + ' ' + req.body.tsData.EvolveTimesheet_StartDateTime.slice(11, 16) + ':00';
            req.body.tsData.EvolveTimesheet_StopDateTime = req.body.tsData.EvolveTimesheet_StopDateTime.slice(6, 10) + '-' + req.body.tsData.EvolveTimesheet_StopDateTime.slice(3, 5) + '-' + req.body.tsData.EvolveTimesheet_StopDateTime.slice(0, 2) + ' ' + req.body.tsData.EvolveTimesheet_StopDateTime.slice(11, 16) + ':00';
            let addTs = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.addEditedTimeSheet(req.body.tsData, req.EvolveUser_ID);
            if (addTs instanceof Error || addTs.rowsAffected < 1) {
                ...
                for (let i = 0; i < req.body.updateArray.length; i++) {
                    if (error == false) {
                        ...
                        let updateSheetOnDelete = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.updateSheetOnDelete(req.body.updateArray[i], req.EvolveUser_ID);
                        if (updateSheetOnDelete instanceof Error || updateSheetOnDelete.rowsAffected < 1) {
                            error = true;
                        }
                    }
    ...
    getMachineSheduleComments: async function (req, res) {
        try {
            let comments = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.getMachineSheduleComments(req.body);
    ...
    getPlcDevice: async function (req, res) {
        try {
            let getPlcDevice = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.getPlcDevice(req.body.EvolveMachine_ID);
            console.log();
    ...
    checkInventory: async function (req, res) {
        try {
            let errorMessage = '';
            let checkInventory = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.checkInventoryPallet(req.body);
    ...
    updateIssuedQty: async function (req, res) {
        try {
            ...
            if (req.body.EvolveInventory_SerialNo == undefined || req.body.EvolveInventory_SerialNo == '' || req.body.EvolveInventory_SerialNo == null) {
                Evolve.Log.error(" EERR####: Error while Ineventory Serial Not Found");
                let obj = {
                    statusCode: 400,
                    status: "fail",
                    message: "Ineventory Serial Not Found",
                    result: null
                };
                res.send(obj);
                ...
                // let invList = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.getInvList(req.body);
                ...
                //     for (let i = 0; i < invToIssued.length; i++) {
                //         let updateInv = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.updateInventory(invToIssued[i]);
                ...
                let updateInv = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.updateInventory(req.body);
                if (updateInv instanceof Error || updateInv.rowsAffected < 1) {
                    errorMessage = 'Error While Update Inventory Status'
                } else {
    ...
    getTransHistory: async function (req, res) {
        try {
            ...
            let getTransHistory = await Evolve.App.Services.SmartFactory.CFDLine1ProdBooking.SrvCFDLine1.getTransHistory(req.body);
    ...
    RePrintMaterialProduced: async function (req, res) {
        try {
            ...
            if(Evolve.Config.CFDLINE1LABEL == '' || Evolve.Config.CFDLINE1LABEL == null || Evolve.Config.CFDLINE1LABEL == undefined || Evolve.Config.CFDLINE1LABEL == 'null' || Evolve.Config.CFDLINE1LABEL == 'undefined' || Evolve.Config.CFDLINE1LABEL == 'NOLABEL'){
                let obj = {
                    statusCode: 400,
                    status: "fail",
                    message: "CFD Line LAbel not Defined",
                    result: null
                };
                res.send(obj);
            }else{
                ...
                if( GetPrinterByMachine.rowsAffected > 0){
                    let EvolvePrinter_Code = GetPrinterByMachine.recordset[0].EvolvePrinter_Code;
                    let sendPrintCommand = await Evolve.App.Controllers.Common.ConCommon.AddNewPrintQuee(EvolvePrinter_Code , Evolve.Config.CFDLINE1LABEL , req.body.EvolveTransHistory)
                    let obj = {
                        statusCode: 200,
                        status: "success",
