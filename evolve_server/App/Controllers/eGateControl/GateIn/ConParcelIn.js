...
module.exports = {
  ...
  addParcelData: async function (req, res) {
    try {
      ...
      let lastReference = await Evolve.App.Services.eGateControl.GateIn.SrvParcelIn.getLastReference(req.body);
      if (lastReference instanceof Error) {
        let obj = { statusCode: 400, status: "fail", message: "Error on get last reference number ", result: null };
        res.send(obj);
      }
      else {
        let date = new Date();
        ...
        let addParcel = await Evolve.App.Services.eGateControl.GateIn.SrvParcelIn.addParcelData(req.body, barcode);
        if (addParcel instanceof Error) {
          ...
          let obj = {
            statusCode: 200,
            status: "Parcel In Successfully",
            message: "Parcel In Successfully",
      ...
      Evolve.Log.error(" EERR0150: Error while adding parcel data "+error.message);
      let obj = {
        statusCode: 400,
        status: "fail",
        message: " EERR0150: Error while adding parcel data "+error.message,
        result: null
      };
      res.send(obj);
    }
