...
module.exports = {
    ...
    getSerialNumberList: async function (req, res) {
        try {
            ...
            let count =await Evolve.App.Services.Evolve.SerialNo.SrvListV1.getSerialNumberCount(search);
            let getSerialNumberData = await Evolve.App.Services.Evolve.SerialNo.SrvListV1.getSerialNumberList(start, length,search);
    ...
    addSerialNumber: async function (req, res) {
        try {
            ...
            let result = await Evolve.App.Services.Evolve.SerialNo.SrvListV1.addSerialNumber(req.body);
    ...
    editSerialNumber: async function (req, res) {
        try {
            req.body.EvolveUser_ID = req.EvolveUser_ID;
            let result = await Evolve.App.Services.Evolve.SerialNo.SrvListV1.editSerialNumber(req.body);
            if (result instanceof Error || result.rowsAffected < 1) {
                let obj = {
                    statusCode: 400,
                    status: "fail",
    ...
    getSingleSerialNumber: async function (req, res) {
        try {
            let result = await Evolve.App.Services.Evolve.SerialNo.SrvListV1.getSingleSerialNumber(req.body.EvolveSerial_ID);
            if (result instanceof Error || result.rowsAffected < 1) {
                let obj = {
                    statusCode: 400,
                    status: "fail",
    ...
    getModelList: async function (req, res) {
        try {
            let modelList = await Evolve.App.Services.Evolve.SerialNo.SrvListV1.getModelList();
