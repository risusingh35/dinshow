...
module.exports = {
        ...
        try {
           ...
           // console.log("req.File >>>>", req.files.csvFile)
            ...
            if (req.files.csvFile) {
                ...
                console.log("Evolve.Config.customerMasterCsvPath :",Evolve.Config.customerMasterCsvPath);
                // ./public/csv/customers
                csv.mv(Evolve.Config.customerMasterCsvPath+fileName, async function(error) {
                  if (error){
                    // console.log("Error in File Upload ::", error.message);
                    let obj = { statusCode: 400, status: "fail", message: error.message, result: null };
                    res.send(obj);
                  }else{
                    let custArray = await Evolve.Csv().fromFile(Evolve.Config.customerMasterCsvPath+fileName);
                    let cust_errorStatus = false;
                    for(let i=0; i < custArray.length; i++)
                    {
                        if(cust_errorStatus == false)
                        {
                            let checkCustExits = await Evolve.App.Services.Evolve.CustomerMaster.SrvList.checkCustExits(custArray[i]['Code']);
    ...
    getCustomerList : async function(req, res) {
        try{
            ...
            let customerCount = await Evolve.App.Services.Evolve.CustomerMaster.SrvList.getCustomerCount(search);
            let customers = await Evolve.App.Services.Evolve.CustomerMaster.SrvList.getCustomerList(start ,length , search);
