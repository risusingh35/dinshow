...
module.exports = {
    getAsnList: async function (req, res) {
        try {
            let start = parseInt(req.body.startFrom);
            let length = parseInt(req.body.displayRecord);
            let search = req.body.search;
            let count = await Evolve.App.Services.Wms.ASN.SrvList.getAsnListCount(search ,  req.EvolveUnit_ID);

            let result = await Evolve.App.Services.Wms.ASN.SrvList.getAsnList(start, length, search ,  req.EvolveUnit_ID);


            if (result instanceof Error) {

                Evolve.Log.error(" EERR####: Error while get ASN List ");
                let obj = {
                    statusCode: 400,
                    status: "fail",
                    message: "EERR#### : Error while get ASN List!",
            ...
            Evolve.Log.error(" EERR####: Error while get ASN List " + error.message);
            let obj = {
                statusCode: 400,
                status: "fail",
                message: " EERR####: Error while get ASN List " + error.message,
                result: null
            };
            res.send(obj);
        }
                        ...
                        if (errorMessage == '') {
                            ...
                            for (let i = 0; i < csvDataArray.length; i++) {
                                ...
                                if (errorMessage == '') {
                                    ...
                                    let checkLocation = await Evolve.App.Services.Wms.ASN.SrvList.checkInventoryLocation(csvDataArray[i]['Location']);
                                    ...
                                    if (checkLocation instanceof Error) {
                                        ...
                                        let unitId = await Evolve.App.Services.Wms.ASN.SrvList.checkUnitCode(data.EvolveUnit_Code);
                                        if (unitId instanceof Error) {
                                            ...
                                            let itemID = await Evolve.App.Services.Wms.ASN.SrvList.checkItemCode(data.EvolveItem_Code);
                                            if (itemID instanceof Error) {
                                                ...
                                                let checkInventory = await Evolve.App.Services.Wms.ASN.SrvList.checkInventory(data);
                                                if (checkInventory instanceof Error) {
                                                    ...
                                                    let updateInventory = await Evolve.App.Services.Wms.ASN.SrvList.updateInventory(data);
                                                    ...
                                                    let addInventory = await Evolve.App.Services.Wms.ASN.SrvList.addInventory(data);
    ...
    getItemList: async function (req, res) {
        try {
            ...
            let result = await Evolve.App.Services.Wms.ASN.SrvList.getItemList(data);
            if (result instanceof Error) {
    ...
    getUnitList: async function (req, res) {
        try {
            ...
            let result = await Evolve.App.Services.Wms.ASN.SrvList.getUnitList(data);
            if (result instanceof Error) {
