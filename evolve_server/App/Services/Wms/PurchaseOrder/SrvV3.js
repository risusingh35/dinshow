...
module.exports = {
	...
	gateEntryNoList : async function (gatePassno) {
		try {
			return await Evolve.SqlPool.request()
				.query("SELECT TOP(20) EvolveGate_PassNumber as title, EvolveGate_ID as id FROM EvolveGate WHERE EvolveGate_PassNumber LIKE '%" + gatePassno + "%'")
		} catch (error) {

			Evolve.Log.error("Error while getting entry no list "+error.message);
			return new Error("Error while getting entry no list "+error.message);

		}
	},

	getDetailsByGateNo: async function (data) {
		// console.log("data>>>", data);
		try {
			return await Evolve.SqlPool.request()
				.input('EvolveGate_ID', Evolve.Sql.Int, data.EvolveGate_ID)
				.query(" SELECT  *, convert(varchar, EvolveGate_CreatedAt, 103) as gateEntryDate  FROM   EvolveGate WHERE  EvolveGate_ID =@EvolveGate_ID ");
		} catch (error) {

			Evolve.Log.error("Error while get details by Gate Number "+error.message);
			return new Error("Error while get details by Gate Number "+error.message);
		}
	},
	...
	receivePurchaseOrderV2: async function (data) {
		try {
			...
			console.log("Emtere >>>>>>>>>>>>>>>>  receivePurchaseOrderV2"  )
			...
			if (po_details instanceof Error || po_details.rowsAffected < 1) {
				...
				console.log("Entered for insert inv?>>>>>>>>>>>>")
				let item_id = po_details.recordset[0].EvolveItem_ID;
					...
					let inserted_inventory = await Evolve.SqlPool.request()
						...
						.query('INSERT INTO EvolveInventory (EvolveInventory_IsPicked ,EvolveCompany_ID ,EvolveUnit_ID,EvolveInventory_LotNumber ,EvolveInventory_CustLotRef ,EvolveItem_ID, EvolveInventory_PostingStatus, EvolveInventory_QtyOnHand,EvolveInventory_RefNumber,EvolveInventory_CreatedUser,EvolveInventory_CreatedAt,EvolveLocation_ID, EvolveInventory_Status,EvolveInventory_ExpireDateTime,EvolveInventory_ReceiptDate , EvolveTranstype_ID,EvolveInventory_QtyAllocated,EvolveInventory_QtyAvailable ,EvolveInventory_Attributes , EvolveInventory_IsTaskCompleted) VALUES (@EvolveInventory_IsPicked , @EvolveCompany_ID ,@EvolveUnit_ID,@EvolveInventory_LotNumber ,@EvolveInventory_CustLotRef ,@EvolveItem_ID, @EvolveInventory_PostingStatus, @EvolveInventory_QtyOnHand,@EvolveInventory_RefNumber,@EvolveInventory_CreatedUser,@EvolveInventory_CreatedAt,@EvolveLocation_ID,@EvolveInventory_Status,@EvolveInventory_ExpireDateTime , @EvolveInventory_ReceiptDate,@EvolveTranstype_ID,@EvolveInventory_QtyAllocated,@EvolveInventory_QtyAvailable ,@EvolveInventory_Attributes , 0);select @@IDENTITY AS \'inserted_id\'');
