...
module.exports = {
    ...
    getInventoryListCount: async function (search , condition , EvolveUnit_ID) {
        try {
            return await Evolve.SqlPool.request()
                .input('search', Evolve.Sql.NVarChar, '%' + search + '%')
                .input('EvolveUnit_ID', Evolve.Sql.Int, EvolveUnit_ID)

                .query("select COUNT(einv.EvolveInventory_ID) as count from EvolveInventory einv left join EvolveUnit eu on eu.EvolveUnit_ID = einv.EvolveUnit_ID  left join EvolveItem ei on ei.EvolveItem_ID = einv.EvolveItem_ID left join EvolveLocation el on el.EvolveLocation_ID = einv.EvolveLocation_ID LEFT JOIN EvolveUom euom ON einv.EvolveUom_ID = euom.EvolveUom_ID WHERE "+  condition + "(eU.EvolveUnit_Code LIKE @search OR  ei.EvolveItem_Part LIKE @search) AND  einv.EvolveUnit_ID = @EvolveUnit_ID")
    ...
    getInventoryList: async function (start, length, search ,condition , EvolveUnit_ID) {
        try {
            return await Evolve.SqlPool.request()
                ...
                .query("select euom.EvolveUom_Uom ,  einv.* , eu.EvolveUnit_Code , ei.EvolveItem_Part , el.EvolveLocation_Code from EvolveInventory einv left join EvolveUnit eu on eu.EvolveUnit_ID = einv.EvolveUnit_ID  left join EvolveItem ei on ei.EvolveItem_ID = einv.EvolveItem_ID left join EvolveLocation el on el.EvolveLocation_ID = einv.EvolveLocation_ID LEFT JOIN EvolveUom euom ON einv.EvolveUom_ID = euom.EvolveUom_ID where" + condition + "(eU.EvolveUnit_Code LIKE @search OR  ei.EvolveItem_Part LIKE @search) AND  einv.EvolveUnit_ID = @EvolveUnit_ID  ORDER BY  einv.EvolveInventory_ID DESC OFFSET @start ROWS FETCH NEXT @length ROWS ONLY")
