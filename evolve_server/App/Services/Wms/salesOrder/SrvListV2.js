...
module.exports = {
  ...
  getAllSalesOrderListCount: async function (search, EvolveUnit_ID) {
    try {
      return await Evolve.SqlPool.request()
        .input('search', Evolve.Sql.NVarChar, '%' + search + '%')
        .input('EvolveUnit_ID', Evolve.Sql.NVarChar, EvolveUnit_ID)

        .query('SELECT count(es.EvolveSalesOrder_ID) as count  FROM EvolveSalesOrder es LEFT JOIN  EvolveCustomer ec on ec.EvolveCustomer_ID = es.EvolveCustomer_ID WHERE  es.EvolveUnit_ID = @EvolveUnit_ID AND ( es.EvolveSalesOrder_Number LIKE @search or ec.EvolveCustomer_Code LIKE @search ) ');
    } catch (error) {
  ...
  getAllSalesOrderList: async function (start, length, search, EvolveUnit_ID) {
    try {
      return await Evolve.SqlPool.request()
  ...
  getSalesQuoteListCount: async function (search, subQuery, releasedSq) {
    try {
      ...
      if (releasedSq) {
        ...
        condition = " AND esq.EvolveSalesOrder_Status = 'QADRELEASED' "
      ...
      subQuery += condition;
      let query = "SELECT  COUNT(EvolveSalesOrder_ID) as count  FROM EvolveSalesOrder esq, EvolveCustomer ec WHERE esq.EvolveSalesOrder_Customer_ID = ec.EvolveCustomer_ID  " + subQuery + " AND EvolveSalesOrder_Number LIKE @search";
      return await Evolve.SqlPool.request()
        .input('search', Evolve.Sql.NVarChar, '%' + search + '%')
  ...
  getSalesQuoteList: async function (start, length, search, subQuery, releasedSq) {
    try {
      ...
      let query = "  SELECT  esq.*, ec.EvolveCustomer_Code , ec.EvolveCustomer_Name , convert(varchar, esq.EvolveSalesOrder_CreatedAt, 103)  as dateCreated , LTRIM(SUBSTRING(CONVERT(VARCHAR(20),   CONVERT(DATETIME, esq.EvolveSalesOrder_CreatedAt), 22), 10, 5) +  RIGHT(CONVERT(VARCHAR(20),   CONVERT(DATETIME, esq.EvolveSalesOrder_CreatedAt), 22), 3)) as timeCreated   FROM  EvolveSalesOrder esq, EvolveCustomer ec   WHERE esq.EvolveCustomer_ID = ec.EvolveCustomer_ID  AND EvolveSalesOrder_Number LIKE @search ORDER BY EvolveSalesOrder_ID DESC OFFSET @start ROWS FETCH NEXT @length ROWS ONLY"
  ...
  submitToApprovelProcess: async function (data) {
    try {
      ...
      return await Evolve.SqlPool.request()
        ...
        .input('EvolveApprovalProcess_PrimaryID', Evolve.Sql.Int, data.EvolveSalesOrder_ID)
  ...
  updateSalesQuoteStatus: async function (data) {
    try {
      ...
      return await Evolve.SqlPool.request()
        ...
        .input('EvolveSalesOrder_SubmitDate', Evolve.Sql.NVarChar, datetime)
        ...
        .query(' UPDATE EvolveSalesOrder SET EvolveApprovalMatrix_ID=@EvolveApprovalMatrix_ID , EvolveSalesOrder_Status = @EvolveSalesOrder_Status,EvolveSalesOrder_SubmitDate=@EvolveSalesOrder_SubmitDate, EvolveSalesOrder_UpdatedAt = @EvolveSalesOrder_UpdatedAt, EvolveSalesOrder_UpdatedUser = @EvolveSalesOrder_UpdatedUser WHERE EvolveSalesOrder_ID = @EvolveSalesOrder_ID ');
    } catch (error) {
  ...
  matchSqDetails: async function (data) {
    try {
      let query = "  SELECT  EvolveSalesOrder_ID  FROM  EvolveSalesOrder  WHERE  EvolveSalesOrder_ID= @EvolveSalesOrder_ID " + data.queryStr
  ...
  deleteSQHeadDetails: async function (EvolveSalesOrder_ID) {
    try {
      return await Evolve.SqlPool.request()
        .input('EvolveSalesOrder_ID', Evolve.Sql.Int, EvolveSalesOrder_ID)
        .query('Delete FROM  EvolveSalesOrder  WHERE EvolveSalesOrder_ID =@EvolveSalesOrder_ID ')
  ...
  deleteSQLineDetails: async function (EvolveSalesOrder_ID) {
    try {
      return await Evolve.SqlPool.request()
        .input('EvolveSalesOrder_ID', Evolve.Sql.Int, EvolveSalesOrder_ID)
        .query('Delete FROM  EvolveSalesOrderDetails  WHERE EvolveSalesOrder_ID =@EvolveSalesOrder_ID ')
  ...
  getSingelSalesQuoteHead: async function (EvolveSalesOrder_ID) {
    try {
      return await Evolve.SqlPool.request()
        .input('EvolveSalesOrder_ID', Evolve.Sql.Int, EvolveSalesOrder_ID)
        .query("SELECT esq.*   , (SELECT  esp.EvolveShipTo_Code FROM  EvolveShipTo esp  ,  EvolveSalesOrder esqi	WHERE esqi.EvolveSalesOrder_ShipTo = esp.EvolveShipTo_ID 	AND esqi.EvolveSalesOrder_ID = @EvolveSalesOrder_ID)  as shipTo ,(SELECT  esp.EvolveShipTo_Code FROM  EvolveShipTo esp  ,  EvolveSalesOrder esqi 	WHERE esqi.EvolveSalesOrder_BillTo = esp.EvolveShipTo_ID AND esqi.EvolveSalesOrder_ID = @EvolveSalesOrder_ID)  as billTo  ,		(SELECT egcm.EvolveGenericCodeMaster_Value  FROM EvolveGenericCodeMaster egcm , EvolveSalesOrder esqi	WHERE egcm.EvolveGenericCodeMaster_Key ='Channel' 	AND egcm.EvolveGenericCodeMaster_ID = esqi.EvolveSalesOrder_Channel_ID AND 	esqi.EvolveSalesOrder_ID = @EvolveSalesOrder_ID) 	as channel , (SELECT egcm.EvolveGenericCodeMaster_Value  FROM EvolveGenericCodeMaster egcm , EvolveSalesOrder esqi	WHERE egcm.EvolveGenericCodeMaster_Key ='Taxenv' 	AND egcm.EvolveGenericCodeMaster_ID = esqi.EvolveSalesOrder_TaxEnv_ID AND 	esqi.EvolveSalesOrder_ID = @EvolveSalesOrder_ID) as taxEnv ,	(SELECT egcm.EvolveGenericCodeMaster_Value  FROM EvolveGenericCodeMaster egcm , EvolveSalesOrder esqi	WHERE egcm.EvolveGenericCodeMaster_Key ='MOD'  	AND egcm.EvolveGenericCodeMaster_ID = esqi.EvolveSalesOrder_Channel_ID	AND esqi.EvolveSalesOrder_ID = @EvolveSalesOrder_ID) as modeOfDelivery 	, convert(varchar, getdate(), 105) as currentDate ,	convert(varchar,esq.EvolveSalesOrder_ReleaseDate, 105) as releaseDate  ,	convert(varchar,esq.EvolveSalesOrder_UpdatedAt, 105) as updatedDate , 	convert(varchar,esq.EvolveSalesOrder_SubmitDate, 105) as submitDate ,	eu.EvolveUnit_Code , ec.EvolveCustomer_Code , ec.EvolveCustomer_Name , ec.EvolveCustomer_Currency ,  ect.EvolveCreditTerms_Code   , ep.EvolveProject_Code , etc.EvolveTaxClass_Code		FROM    EvolveUnit eu , EvolveCustomer ec  ,  EvolveCreditTerms ect    ,EvolveProject ep   , EvolveSalesOrder esq  LEFT JOIN   EvolveTaxClass etc  ON  esq.EvolveSalesOrder_TaxClass_ID = etc.EvolveTaxClass_ID	WHERE EvolveSalesOrder_ID = @EvolveSalesOrder_ID 	AND esq.EvolveUnit_ID = eu.EvolveUnit_ID 		AND esq.EvolveSalesOrder_Customer_ID = ec.EvolveCustomer_ID	AND esq.EvolveCreditTerms_ID = ect.EvolveCreditTerms_ID AND esq.EvolveSalesOrder_Project_ID = 	ep.EvolveProject_ID");
    } catch (error) {
  ...
  getSingelSalesQuoteDetails: async function (EvolveSalesOrder_ID) {
    try {
      return await Evolve.SqlPool.request()
        .input('EvolveSalesOrder_ID', Evolve.Sql.Int, EvolveSalesOrder_ID)
        // .query("  SELECT esqd.*, convert(varchar, esqd.EvolveSalesOrderDetails_ReqdDate, 103)  as reqDate ,   convert(varchar, esqd.EvolveSalesOrderDetails_PromiseDate, 103)  as promiseDate,   convert(varchar, esqd.EvolveSalesOrderDetails_DueDate, 103)  as dueDate  , ei.EvolveItem_Code ,ei.EvolveItem_ItemUnitPrice  FROM EvolveSalesOrderDetails  esqd , EvolveItem ei  WHERE esqd.EvolveSalesOrder_ID = @EvolveSalesOrder_ID   AND esqd.EvolveItem_ID = ei.EvolveItem_ID");

        .query("  SELECT esqd.EvolveSalesOrderDetails_ItemUnitPrice as EvolveItem_ItemUnitPrice , esqd.EvolveSalesOrderDetails_ID , esqd.EvolveSalesOrder_ID ,esqd.EvolveSalesOrderDetails_LineNo ,esqd.EvolveSalesOrderDetails_TaxEnv_ID , esqd.EvolveItem_ID ,esqd.EvolveSalesOrderDetails_Qty,esqd.EvolveSalesOrderDetails_CustomerUnitPrice ,esqd.EvolveSalesOrderDetails_CustomerDiscount,esqd.EvolveTaxClass_ID ,esqd.EvolveSalesOrderDetails_Comments , convert(varchar, esqd.EvolveSalesOrderDetails_ReqdDate, 105)  as EvolveSalesOrderDetails_ReqdDate ,   convert(varchar, esqd.EvolveSalesOrderDetails_PromiseDate, 105)  as EvolveSalesOrderDetails_PromiseDate,   convert(varchar, esqd.EvolveSalesOrderDetails_DueDate, 105)  as EvolveSalesOrderDetails_DueDate  , ei.EvolveItem_Code FROM EvolveSalesOrderDetails  esqd , EvolveItem ei  WHERE esqd.EvolveSalesOrder_ID = @EvolveSalesOrder_ID   AND esqd.EvolveItem_ID = ei.EvolveItem_ID");
    } catch (error) {
  ...
  getFullSoList: async function (condition) {
    try {
      return await Evolve.SqlPool.request()
        .query("SELECT  EvolveSalesOrder_ID  , EvolveSalesOrder_Number  FROM EvolveSalesOrder" + condition);
    } catch (error) {

      Evolve.Log.error(" EERR####: Error While Get Full So List " + error.message);
      return new Error(" EERR####: Error While Get Full So List " + error.message);
    }
  },
