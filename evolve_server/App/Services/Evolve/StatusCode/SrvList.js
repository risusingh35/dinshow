...
module.exports = {
    ...
    getStausCodeListCount: async function (search) {
        try {
            return await Evolve.SqlPool.request()
            .input('search', Evolve.Sql.NVarChar, '%'+search+'%')
            .query('  SELECT COUNT(EvolveStatusCodeMstr_Id) as count  FROM EvolveStatusCodeMstr WHERE EvolveStatusCodeMstr_Code LIKE @search');
        } catch (error) {
            Evolve.Log.error(" EERR3037: Error while get status code count "+error.message);
            return new Error(" EERR3037: Error while get status code count "+error.message);
        }
    },

    getStausCodeList: async function (start, length ,search) {
        try {
            return await Evolve.SqlPool.request()
