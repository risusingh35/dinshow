...
module.exports = {
    ...
    getSerialNumberCount : async function (search) {
        try {
          ...
          .query("SELECT COUNT(es.EvolveSerial_ID) as count FROM EvolveSerial es, EvolveUnit eu, EvolveItem ei WHERE es.EvolveModel_ID = ei.EvolveItem_ID AND es.EvolveUnit_ID = eu.EvolveUnit_ID AND EvolveSerial_Code LIKE @search")
        } catch (error) {
    ...
    getSerialNumberList: async function (start, length,search) {
        try {
            ...
            return await Evolve.SqlPool.request()
                ...
                .input('search', Evolve.Sql.NVarChar, '%'+search+'%')
                .query('SELECT es.*, eu.EvolveUnit_Code, ei.EvolveItem_Code FROM EvolveSerial es, EvolveUnit eu, EvolveItem ei WHERE es.EvolveModel_ID = ei.EvolveItem_ID AND es.EvolveUnit_ID = eu.EvolveUnit_ID AND es.EvolveSerial_Code LIKE @search ORDER BY es.EvolveSerial_ID DESC OFFSET @start ROWS FETCH NEXT @length ROWS ONLY');
        } catch (error) {
    ...
    getSingleSerialNumber: async function (EvolveSerial_ID) {
        try {
            return await Evolve.SqlPool.request()
                ...
                .query('SELECT es.*, eu.EvolveUnit_Code, ei.EvolveItem_ID FROM EvolveSerial es, EvolveUnit eu, EvolveItem ei WHERE es.EvolveModel_ID = ei.EvolveItem_ID AND es.EvolveUnit_ID = eu.EvolveUnit_ID AND es.EvolveSerial_ID = @EvolveSerial_ID');
        } catch (error) {
