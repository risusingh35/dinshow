...
module.exports = {
  ...
  getAvailableInventory: async function (data) {
    try {
      ...
      return await Evolve.SqlPool.request()
        .input("EvolveItem_ID", Evolve.Sql.Int, data.EvolveItem_ID)
        .query("SELECT TOP(10) CAST(0 AS int) AS isSelected , einv.*  , ei.EvolveItem_Part ,  eloc.EvolveLocation_Code  FROM    EvolveInventory einv ,  EvolveItem ei ,  EvolveLocation eloc   WHERE eloc.EvolveLocation_ID = einv.EvolveLocation_ID AND ei.EvolveItem_ID = einv.EvolveItem_ID AND ei.EvolveItem_ID=@EvolveItem_ID AND einv.EvolveInventory_Status ='AVAILABLE'")
    } catch (error) {
