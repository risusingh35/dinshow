...
module.exports = {
  ...
  //   }
  // },
  getWoList: async function (search) {
    try {
      let query = "  SELECT TOP(20) (EvolveProdOrders_OrderNo+'-'+EvolveProdOrders_OrderID) as title, EvolveProdOrders_ID as id FROM   EvolveProdOrders  WHERE  (EvolveProdOrders_OrderNo  LIKE '%" + search + "%'  OR EvolveProdOrders_OrderID  LIKE '%" + search + "%') AND  EvolveProdOrders_Status='published' "
  ...
  getWoBomDetails: async function (EvolveProdOrders_ID) {
    try {
      ...
      console.log("getWoBomDetails??????" ,EvolveProdOrders_ID  )
      return await Evolve.SqlPool.request()
        .input("EvolveProdOrders_ID", Evolve.Sql.Int, EvolveProdOrders_ID)
  ...
  updateSalesOrderPickListStatus: async function (data) {
    try {
      let dateTime = await Evolve.App.Controllers.Unit.unitControllers.getDateTime();
      return await Evolve.SqlPool.request()
        .input("EvolveSalesOrder_ID", Evolve.Sql.Int,data.EvolveSalesOrder_ID)
        .input("EvolvePickList_ID", Evolve.Sql.Int, data.EvolvePickList_ID)
        .input("EvolveSalesOrder_IsPicklistGenerated", Evolve.Sql.Int, 1)
        ...
        .query("UPDATE EvolveSalesOrder SET EvolveSalesOrder_IsPicklistGenerated = @EvolveSalesOrder_IsPicklistGenerated,  EvolvePickList_ID=@EvolvePickList_ID , EvolveSalesOrder_CreatedUser = @EvolveSalesOrder_CreatedUser  , EvolveSalesOrder_UpdatedUser =@EvolveSalesOrder_UpdatedUser ,  EvolveSalesOrder_CreatedAt =@EvolveSalesOrder_CreatedAt ,  EvolveSalesOrder_UpdatedAt =@EvolveSalesOrder_UpdatedAt WHERE EvolveSalesOrder_ID=@EvolveSalesOrder_ID ")
    } catch (error) {
      Evolve.Log.error(" EERR1799: Erorr while update Sales order picklist status " + error.message);
      return new Error(" EERR1799: Erorr while update Sales order picklist status " + error.message);
    }
  },
  ...
  addPickListDetails: async function (data) {
    try {
      ...
      return await Evolve.SqlPool.request()
        ...
        .input("EvolvePickListDetail_ReqQty", Evolve.Sql.NVarChar, data.PickList == 'WO' ? data.EvolveProdOrdersDetails_QtyReq : data.EvolveSalesOrderLine_Qty)
