...
module.exports = {
    ...
    addKneeAudit: async function (data,s1,s2) {
        try {
            console.log("data :",data);
            return await Evolve.SqlPool.request()
                .input('EvolveKneeAudit_Barcode', Evolve.Sql.NVarChar, data.parentSerial)
                .input('EvolveKneeAudit_S1', Evolve.Sql.Int, s1)
                .input('EvolveKneeAudit_S2', Evolve.Sql.Int, s2)
                .query("INSERT INTO EvolveKneeAudit (EvolveKneeAudit_Barcode , EvolveKneeAudit_S1 , EvolveKneeAudit_S2) VALUES(@EvolveKneeAudit_Barcode , @EvolveKneeAudit_S1 , @EvolveKneeAudit_S2)")
        } catch (error) {
            Evolve.Log.error(error.message);
            return new Error(error.message);
        }
    ...
    getKneeBolsterChildList: async function (EvolveProdOrdersDetail_ID) {
        try {
          return await Evolve.SqlPool.request()
            .input("EvolveProdOrdersDetail_ID",Evolve.Sql.Int,EvolveProdOrdersDetail_ID)
            .query("SELECT ei.EvolveItem_ID , ei.EvolveItem_Code , epoc.EvolveProdOrdersDetailChild_QtyPer , epoc.EvolveProdOrdersDetailChild_QtyScanned , epoc.EvolveProdOrdersDetailChild_Scanned , ei.EvolveItem_CustPart , ei.EvolveItem_Desc,ei.EvolveItem_Unique FROM EvolveProdOrdersDetailChild epoc , EvolveItem ei WHERE epoc.EvolveProdOrdersDetail_ID = @EvolveProdOrdersDetail_ID AND ei.EvolveItem_ID = epoc.EvolveProdOrdersDetailChild_ChildItemId ORDER BY EvolveProdOrdersDetailChild_Scanned DESC ")
        } catch (error) {
            Evolve.Log.error(error.message);
            return new Error(error.message);
        }
    },
    startKneeBolster: async function (EvolveProdOrdersDetail_Serial) {
        try {
            let date = new Date();
            let dataTime = date.getFullYear()+'-'+(date.getMonth()+1)+'-'+date.getDate()+' '+date.getHours()+':'+date.getMinutes()+':'+date.getSeconds();
            return await Evolve.SqlPool.request()
    ...
    getKneeBolsterWoList: async function () {
        try {
          return await Evolve.SqlPool.request()
            .query("SELECT epod.EvolveProdOrdersDetail_Serial,ei.EvolveItem_Code,epo.EvolveProdOrders_OrderId FROM EvolveProdOrdersDetail epod, EvolveProdOrders epo, EvolveItem ei  WHERE  epod.EvolveProdOrders_ID = epo.EvolveProdOrders_ID AND epod.EvolveProdOrdersDetail_PrvSeq = 13 AND EvolveProdOrdersDetail_NxtSeq = 13 AND (EvolveProdOrdersDetail_Status ='In Process' OR EvolveProdOrdersDetail_Status ='In Queue') AND epo.EvolveItem_ID = ei.EvolveItem_ID ")
        } catch (error) {
            Evolve.Log.error(error.message);
            return new Error(error.message);
        }
    },
    getKneeBolsterCompletedTriggers: async function () {
        try {
          return await Evolve.SqlPool.request()
            .query("SELECT count(epod.EvolveProdOrdersDetail_Serial) as count FROM EvolveProdOrdersDetail epod, EvolveProdOrders epo, EvolveItem ei  WHERE  epod.EvolveProdOrders_ID = epo.EvolveProdOrders_ID AND epod.EvolveProdOrdersDetail_PrvSeq = 13 AND EvolveProdOrdersDetail_NxtSeq= 13 AND EvolveProdOrdersDetail_Status ='Completed' AND epo.EvolveItem_ID = ei.EvolveItem_ID AND CAST(epod.EvolveProdOrdersDetail_CreatedAt as date) >= FORMAT(getdate(), 'yyyy-MM-dd')")
        } catch (error) {
            Evolve.Log.error(error.message);
            return new Error(error.message);
        }
    },
    kneeBolsterCheckAudit: async function () {
        try {
          return await Evolve.SqlPool.request()
            .query("SELECT * FROM EvolveKneeAudit")
        } catch (error) {
            Evolve.Log.error(error.message);
            return new Error(error.message);
        }
    ...
    TruncateKneeAudit: async function () {
        try {
          return await Evolve.SqlPool.request()
            .query("TRUNCATE TABLE EvolveKneeAudit")
        } catch (error) {
            Evolve.Log.error(error.message);
            return new Error(error.message);
        }
    ...
    getKneeNextBarcode: async function (prvBarcode) {
        try {
            //console.log(prvBarcode);
          return await Evolve.SqlPool.request()
            .input('EvolveProdOrdersDetail_Serial',Evolve.Sql.NVarChar,prvBarcode)
            .query("SELECT TOP(1) * FROM EvolveProdOrdersDetail WHERE EvolveProdOrdersDetail_Serial > @EvolveProdOrdersDetail_Serial AND EvolveProdOrdersDetail_PrvSeq = 13 AND EvolveProdOrdersDetail_NxtSeq = 13 AND EvolveProdOrdersDetail_Status = 'In Queue' AND (EvolveProdOrdersDetail_Status != 'In Completed' OR EvolveProdOrdersDetail_Status = 'Completed') ");
        } catch (error) {
            Evolve.Log.error(error.message);
            return new Error(error.message);
        }
