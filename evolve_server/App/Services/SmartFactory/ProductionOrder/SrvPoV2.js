...
module.exports = {
    ...
    startWorkOrder: async function (data) {
        try {
            ...
            if(wo_data instanceof Error || wo_data.rowsAffected < 1) {
                let obj = { statusCode: 400, status: "fail", message: wo_data.message , result: null };
                res.send(obj);
            } else {
                ...
                if(get_processTemp instanceof Error || get_processTemp.rowsAffected < 1){
                    let obj = { statusCode: 400, status: "fail", message: get_processTemp.message , result: null };
                    res.send(obj);
                } else {
                    var seq = get_processTemp.recordset[0].EvolveProcessTemp_Seq;

                    console.log("wo_data.recordset[0].EvolveSerial_Next>>>>" , wo_data.recordset[0].EvolveSerial_Next)
                    var str = "" + wo_data.recordset[0].EvolveSerial_Next;
                    console.log("str>>>>" ,  str)
                    var pad = "00000";
                    var sr_end = pad.substring(0, pad.length - str.length) + str; //0001
                    console.log("sr_end>>>" ,  sr_end)
                    var dateObj = new Date();
                    var month = dateObj.getMonth() + 1; //months from 1-12
                    var day = dateObj.getDate();
                    var year = dateObj.getFullYear().toString().substr(-2);
                    ...
                    for (var qty = 1; qty <= wo_data.recordset[0].EvolveProdOrders_Quantity; qty++) {
                        if(wo_data.recordset[0].EvolveItemGroup_ID == 2 || wo_data.recordset[0].EvolveItemGroup_ID == 3 || wo_data.recordset[0].EvolveItemGroup_ID == 4  ){
                            ...
                            if(wo_data.recordset[0].EvolveItem_Type == 'FRRH') {
                                ...
                                prefix = '1';
                            }else if(wo_data.recordset[0].EvolveItem_Type == 'IpFlaming') {
                                prefix = '1';
                            }
                        ...
                        } else if(wo_data.recordset[0].EvolveItemGroup_ID == 5 || wo_data.recordset[0].EvolveItemGroup_ID == 8 ){
                            let scanReq = 0;
                            if (month == 10) {
                                month = "A";
                            } else if (month == 11) {
                            ...
                            if(wo_data.recordset[0].EvolveItem_Type == 'FRRH') {
                                ...
                                prefix = '1';
                            }else if(wo_data.recordset[0].EvolveItem_Type == 'IpAssy') {
                                prefix = '1';
                            }else if(wo_data.recordset[0].EvolveItem_Type == 'HPLAMINATION'){
                                prefix = '2';
                            ...
                            for(let j=0;j < get_partBom.rowsAffected ; j++) {
                                ...
                                let check_scan = await Evolve.SqlPool.request()
                                    .input('EvolveItem_ID',Evolve.Sql.Int,get_partBom.recordset[j].EvolvePartBom_CompItem_ID)
                                    .query('  SELECT ei.EvolveItem_IsScan   , eig.EvolveItemGroup_Name  FROM EvolveItem ei , EvolveItemGroup eig WHERE ei.EvolveItem_ID = @EvolveItem_ID AND ei.EvolveItemGroup_ID = eig.EvolveItemGroup_ID');
								let cmp_Qty = 0;
								console.log('item_scan :',check_scan.recordset[0].EvolveItem_IsScan);
                                if(check_scan.recordset[0].EvolveItem_IsScan == true){
                                    cmp_Qty = 0;
                                ...
                                let sequenceId ;
                                if(check_scan.recordset[0].EvolveItemGroup_Name  == 'IP Flaming'){
                                    sequenceId = 21 ;
                                }else if(check_scan.recordset[0].EvolveItemGroup_Name  == 'IP Foaming'){
                                    sequenceId = 22 ;

                                }else if(check_scan.recordset[0].EvolveItemGroup_Name  == 'HP Lamination'){
                                    sequenceId = 23 ;
                                }else{
                                    sequenceId = null ;
                                ...
                                let insert_ChPartBom = await Evolve.SqlPool.request()
                                    ...
                                    .input('EvolveProdOrdersDetailChild_UpdatedUser',Evolve.Sql.Int, data.EvolveUser_ID)
                                    .input('EvolveProdOrdersDetailChild_UpdatedAt',Evolve.Sql.NVarChar, dataTime)
                                    .input('EvolveProdOrdersDetailChild_parentSequenceId',Evolve.Sql.Int, sequenceId)
                                    .query('INSERT INTO EvolveProdOrdersDetailChild (EvolveProdOrdersDetailChild_ParentItemID , EvolveProdOrdersDetailChild_ChildItemId , EvolveProdOrdersDetail_ID , EvolveProdOrdersDetailChild_QtyPer , EvolveProdOrdersDetailChild_QtyScanned , EvolveProdOrdersDetailChild_Scanned , EvolveProdOrdersDetailChild_CreatedUser , EvolveProdOrdersDetailChild_CreatedAt , EvolveProdOrdersDetailChild_UpdatedUser , EvolveProdOrdersDetailChild_UpdatedAt ,EvolveProdOrdersDetailChild_parentSequenceId) VALUES (@EvolveProdOrdersDetailChild_ParentItemID , @EvolveProdOrdersDetailChild_ChildItemId , @EvolveProdOrdersDetail_ID , @EvolveProdOrdersDetailChild_QtyPer ,@EvolveProdOrdersDetailChild_QtyScanned , @EvolveProdOrdersDetailChild_Scanned , @EvolveProdOrdersDetailChild_CreatedUser , @EvolveProdOrdersDetailChild_CreatedAt , @EvolveProdOrdersDetailChild_UpdatedUser , @EvolveProdOrdersDetailChild_UpdatedAt ,@EvolveProdOrdersDetailChild_parentSequenceId)');
                                //console.log(get_partBom.recordset[j].EvolvePartBom_CompItem_ID+" "+cmp_Qty)
                            }
                            let updateProdSerial = await Evolve.SqlPool.request()
                                .input('EvolveProdOrdersDetail_ID',Evolve.Sql.Int,add_serialNo.recordset[0].inserted_id)
    ...
    getWorkOrderDetail: async function (EvolveProdOrdersDetail_ID) {
        try {
            return await Evolve.SqlPool.request()
    ...
    closeWorkOrder : async function (data){
		try {
            let updateProdOrdersDetails =  await Evolve.SqlPool.request()
                .input('EvolveProdOrders_ID', Evolve.Sql.Int, parseInt(data.EvolveProdOrders_ID))
            ...
            if(updateProdOrdersDetails instanceof Error || updateProdOrdersDetails.rowsAffected < 1){
                ...
                // res.send(obj);
                Evolve.Log.error("Error while update production order details")
            }else{
                return await Evolve.SqlPool.request()
                    .input('EvolveProdOrders_ID', Evolve.Sql.Int, parseInt(data.EvolveProdOrders_ID))
