...
module.exports = {
    ...
    getApprovalListCount: async function (search ,EvolveUser_ID,displayAll) {
        try {

            let query ;
            if(displayAll){
                query = "SELECT COUNT(eap.EvolveApprovalProcess_ID) as count FROM EvolveApprovalProcess eap  ,  EvolveApprovalMatrix eapm     WHERE eap.EvolveApprovalMatrix_ID = eapm.EvolveApprovalMatrix_ID "
            }else{
                query = " SELECT COUNT(eap.EvolveApprovalProcess_ID) as count FROM EvolveApprovalProcess eap  ,  EvolveApprovalMatrix eapm     WHERE eap.EvolveApprovalMatrix_ID = eapm.EvolveApprovalMatrix_ID AND eap.EvolveApprovalProcess_CreatedUser = @EvolveUser_ID  "
            }
    ...
    reProcessApprovalProcess: async function (data) {
        try {
            return await Evolve.SqlPool.request()
                .input('EvolveApprovalProcess_ID', Evolve.Sql.Int, data.EvolveApprovalProcess_ID)
                .input('EvolveApprovalProcess_Status', Evolve.Sql.NVarChar, data.EvolveApprovalProcess_Status)
                ...
                .query('UPDATE EvolveApprovalProcess SET  EvolveApprovalProcess_Status =@EvolveApprovalProcess_Status ,  EvolveApprovalProcess_ErrorCode=@EvolveApprovalProcess_ErrorCode WHERE  EvolveApprovalProcess_ID=@EvolveApprovalProcess_ID');
        } catch (error) {
            Evolve.Log.error(" EERR####: Error while upddate process status "+error.message);
            return new Error(" EERR####: Error while upddate process status "+error.message);
        }
    },

    getGateInDetails: async function (EvolveGate_ID) {
        try {
            return await Evolve.SqlPool.request()
            ...
            Evolve.Log.error(" EERR####: Error while get Gate entry details "+error.message);
            return new Error(" EERR####: Error while get Gate entry details "+error.message);
        }
    },

    gateHeadDetails: async function (data) {
        try {
            return await Evolve.SqlPool.request()
                .input('EvolveGate_ID', Evolve.Sql.Int, data.EvolveGate_ID)
                .query('SELECT  * FROM EvolveGate WHERE EvolveGate_ID=@EvolveGate_ID ');
        } catch (error) {
            Evolve.Log.error(" EERR####: Error while get Gate Entry Head details "+error.message);
            return new Error(" EERR####: Error while get Gate Entry Head details "+error.message);
        }
    },

    gateLineDetails: async function (data) {
        try {
            return await Evolve.SqlPool.request()
                .input('EvolveGate_ID', Evolve.Sql.Int, data.EvolveGate_ID)
                .query('  SELECt egd.* ,  ei.EvolveItem_Part  FROM EvolveGateDetails egd  LEFT JOIN  EvolveItem ei ON  egd.EvolveItem_ID = ei.EvolveItem_ID  WHERE egd.EvolveGate_ID = @EvolveGate_ID ');
        } catch (error) {
            Evolve.Log.error(" EERR####: Error while get Gate Entry Line details "+error.message);
            return new Error(" EERR####: Error while get Gate Entry Line details "+error.message);
        }
    },
