...
module.exports = {
    ...
    getImageUrlAuth: function (req, res, next) {
        ...
        if (validateEvolveData.error) {
             Evolve.Log.error(" EERR2193: Error in getting Image Url Auth "+validateEvolveData.error.toString());
            res.send({
                statusCode: 400,
                status: 'fail',
    ...
    addParcelDataAuth: function (req, res, next) {
        ...
        if (validateEvolveData.error) {
             Evolve.Log.error(" EERR2194: Error in adding Parcel Data Auth "+validateEvolveData.error.toString());
            res.send({
                statusCode: 400,
                status: 'fail',
                message: validateEvolveData.error.toString(),
